...
1. 对于 `load_flashcards` 函数的更改：
   - 优化了选择卡片的顺序，首先选择状态为1或2且下次复习日期为今天的卡片。
   - 检查选定的卡片是否已经存在于 `Table2` 中，如果存在，则从列表中删除。
   - 如果所有选定的卡片都已经存在于 `Table2` 中，然后选择状态为1或2的卡片。

2. 对于 `insert_initial_data` 函数的更改：
   - 从使用集合 `inserted_card_ids` 存储已插入的卡片ID，更改为查询 `Table2` 并将已存在的卡片ID 存储在一个列表中。
   - 这种更改可以更有效地跟踪已插入的卡片ID，并避免重复插入。
